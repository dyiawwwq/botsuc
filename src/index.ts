...
import { registerCommands } from "./discord/deployCommands.js";
...
async function main() {
  ...
  // Self-register slash commands with Discord on every boot. This means
  // commands stay in sync automatically on every deploy — no separate
  // manual step, and no dependency on a Render dashboard field. A failure
  // here is logged but doesn't stop the bot from starting: whatever
  // commands were already registered with Discord from a previous boot
  // will keep working either way.
  try {
    await registerCommands();
  } catch (err) {
    logger.error({ err: err instanceof Error ? err.message : err }, "Command registration failed; continuing startup");
  }
